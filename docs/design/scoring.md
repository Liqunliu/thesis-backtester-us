...
21 个算子组合成 6 个章节，最终输出一个 0-100 分和买入/观望/回避建议。**凭什么这个组合能打分？**
...
Step 3: 核心矛盾     — 最可能亏钱的原因是什么？赚的是什么钱？
Step 4: 锚点校准     — 参照评分锚点，给出分数
Step 5: 输出结论     — 结构化 JSON
```
...
framework:
  synthesis:
    ...
    scoring_rubric:
      ...
      - range: "50-69"
        description: "有一定吸引力但存在明确疑虑（如周期位置不确定、管理层存疑）→ 观望"
      - range: "30-49"
...
三层评分体系在 120 只股票 × 12 个截面（2020-2025）的回测中验证有效：

- 70-84 分买入区间：6m 均收益 +8.2%，胜率 68%（最优区间）
- 评分从 0 到 84 分单调递增，区分度清晰
- 回避信号（≤29 分）73% 后续下跌
