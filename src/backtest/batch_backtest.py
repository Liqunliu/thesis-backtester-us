...
def generate_report(results: List[CrossSectionResult], config: Optional["StrategyConfig"] = None) -> str:
    """生成批量回测统计报告"""
    # 从 config 读取分级名称，动态生成表头
    if config is not None:
        ...
        tier_names = []
        default_label = '不达标'
    all_tier_names = tier_names + [default_label]
    ...
    # 汇总统计
    tier_headers = " | ".join(tier_names) if tier_names else "评级"
    tier_seps = " | ".join(["------"] * len(tier_names)) if tier_names else "------"
    lines.append("## 各截面概览")
    lines.append(f"| 截面日期 | 候选数 | {tier_headers} | 6m均收益 | 6m胜率 | 6m均回撤 |")
    lines.append(f"|----------|--------|{tier_seps}|----------|--------|----------|")

    all_returns_6m = []

    for cs in results:
        if cs.screen_result is None or cs.screen_result.candidates.empty:
            tier_zeros = " | ".join(["-"] * max(len(tier_names), 1))
            lines.append(f"| {cs.cutoff_date} | 0 | {tier_zeros} | - | - | - |")
            continue

        df = cs.candidates_with_returns
        n = len(df)
        tc = df['tier_rating'].value_counts() if 'tier_rating' in df.columns else pd.Series(dtype=int)
        ...
        tier_counts_str = " | ".join(str(tc.get(tn, 0)) for tn in tier_names) if tier_names else str(n)
        lines.append(
            f"| {cs.cutoff_date} | {n} "
            f"| {tier_counts_str} "
    ...
    # 按评级分组统计
    lines.extend(["", "## 按评级分组统计（6个月前向收益）", ""])
    lines.append("| 评级 | 样本数 | 平均收益 | 胜率 | 中位数 |")
    lines.append("|------|--------|----------|------|--------|")

    for rating in all_tier_names:
        rets = []
        for cs in results:
            df = cs.candidates_with_returns
            if df.empty or 'tier_rating' not in df.columns:
                continue
            mask = df['tier_rating'] == rating
...
def main():
    ...
    report = generate_report(results, config=config)
    print(f"\n{'='*60}")
    print(report)
